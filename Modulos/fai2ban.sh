...
# --- COMPATIBILIDAD GENKKOSSH ---
[[ -e /etc/VPS-AGN/controller/controller ]] && source /etc/VPS-AGN/controller/controller
[[ -e /etc/VPS-AGN/tools/functions ]] && source /etc/VPS-AGN/tools/functions
# --------------------------------
 ...
 echo -e "\e[97m          Anti DDOS attacks and SPAM spoofing"
 msg -bar
 if [[ ! -z "$pid_fail" ]]; then
  echo -e "${cor[2]} [1] >${cor[5]} $(fun_trans "Uninstall Fail2ban")"
  echo -e "${cor[2]} [2] >\e[92m $(fun_trans "Look at the log")"
  msg -bar
   while [[ -z ${logxyz} || ${logxyz} != @(1|2) ]]; do
    echo -ne "\033[1;37m$(fun_trans "Select an Option"): " && read logxyz
    tput cuu1 && tput dl1
   done
  case ${logxyz} in
 ...
 echo -e "${cor[5]}        Do you want to install Fail2ban?"
 msg -bar
   while [[ -z ${fail2ban} || ${fail2ban} != @(s|S|n|N|y|Y) ]]; do
    echo -ne "\033[1;37m$(fun_trans "Select an Option") [S/N]: " && read fail2ban
    tput cuu1 && tput dl1
   done

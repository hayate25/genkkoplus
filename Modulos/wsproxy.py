...
# Pass (DEPRECATED - usar sistema de usuarios)
PASS = ''
...
RESPONSE = 'HTTP/1.1 101 VPS Premium\r\n\r\nContent-Length: 104857600000\r\n\r\n'
USE_USER_SYSTEM = True  # Activar sistema de usuarios con HWID
...
class ConnectionHandler(threading.Thread):
    ...
    def run(self):
        try:
            ...
            if hostPort != b'':
                # NUEVO: Sistema de usuarios con HWID
                if USE_USER_SYSTEM:
                ...
                # ANTERIOR: Sistema de contraseña global (backward compatible)
                else:
    ...
    def _authenticate_user(self):
        """NUEVO: Autentica usuario usando el sistema de gestión de usuarios"""
        username = self.findHeader(self.client_buffer, b'X-User')
        password = self.findHeader(self.client_buffer, b'X-Pass')
        hwid = self.findHeader(self.client_buffer, b'X-HWID')
        ...
        hwid_str = hwid.decode('utf-8') if hwid != b'' else None

        success, msg = user_manager.authenticate(username_str, password_str, hwid_str)

        if success:
            self.log += f' - USER:{username_str}'
            if hwid_str:
                self.log += f' HWID:OK'
            self.server.printLog(self.log)
        else:
            self.log += f' - AUTH_FAILED:{msg}'
            self.server.printLog(self.log)
...
def main(host=LISTENING_ADDR, port=LISTENING_PORT):
    print("\n:-------AGN Websocket Proxy by Khaled AGN-------:\n")
